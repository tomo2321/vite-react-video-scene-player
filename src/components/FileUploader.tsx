...
const FileUploader: React.FC<FileUploaderProps> = ({ onVideoLoad, onSubtitlesLoad }) => {
  ...
  const handleSubtitleChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      try {
        ...
        alert(
          `Error parsing subtitle file: ${error instanceof Error ? error.message : 'Unknown error'}. Please check the file format.`
        );
      }
    }
  };
  ...
  return (
    <div className="file-uploader">
      <div className="upload-section">
        <button type="button" onClick={triggerVideoUpload} className="upload-button">
          📹 Load Video
        </button>
        <input
          ref={videoInputRef}
      ...
      <div className="upload-section">
        <button type="button" onClick={triggerSubtitleUpload} className="upload-button">
